...
def init_interview_state(job_objectives: list[dict], total_time: int) -> dict:
    objs = [o for o in job_objectives if (o.get("title") or "").strip()]
    ...
    return {
        ...
        "asked_questions": {o["id"]: [] for o in objs},
        "score_history": {o["id"]: [] for o in objs},
    }
...
def _default_job_objectives() -> list[dict]:
    return [
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        st.latex(r"C := \text{clamp}\left(C + 0.025 \cdot \text{score},\; (-1,\; 1]\right)")
        st.latex(r"T^{\text{spent}} := T^{\text{spent}} + \Delta t")
        st.latex(r"\text{remaining} := \text{remaining} - \Delta t")

        st.markdown("### Unlock Triggers")
        st.latex(r"\text{unlock} := \text{completed}_{\text{focus}} \;\lor\; (\text{last2\_scores} < -3)")
        st.latex(r"\text{if unlock: } \text{focus} := \varnothing")
    ...
    if not st.session_state.job_objectives:
        st.session_state.job_objectives = _default_job_objectives()
    ...
    with col_a:
        if st.button("Start / Reset Interview"):
            st.session_state.interview = init_interview_state(st.session_state.job_objectives, total_time)
    with col_b:
