...
def _generate_question(obj: dict, d_current: int, model_name: str) -> str:
...
def _priority(obj: dict, st_obj: dict) -> float:
    ...
    denom = max(abs(C), 0.05)
    return ((float(I) / denom) * m_depth) + B
...
if st.session_state.interview["running"]:
    ...
    active = []
    for oid in list(st.session_state.interview["active_ids"]):
        obj = objs_by_id.get(oid)
        st_obj = st.session_state.interview["state"].get(oid)
        if not obj or not st_obj:
            continue
        if float(st_obj.get("C", 0.0)) >= 0.8:
            continue
        if float(st_obj.get("Tspent", 0.0)) >= float(st_obj.get("Tcap", 0.0)):
            continue
        active.append(oid)

    st.session_state.interview["active_ids"] = active

    if st.session_state.interview["remaining_time"] <= 0 or not active:
        st.session_state.interview["running"] = False
        st.subheader("Interview ended")
    else:
        prios = []
        for oid in active:
            prios.append((oid, _priority(objs_by_id[oid], st.session_state.interview["state"][oid])))
        prios.sort(key=lambda x: x[1], reverse=True)
        selected_id = prios[0][0]

        st.session_state.interview["current_objective_id"] = selected_id
        sel_obj = objs_by_id[selected_id]
        sel_state = st.session_state.interview["state"][selected_id]
        ...
        st.markdown(f"**Current objective:** {sel_obj['title']}")
        st.markdown(
        ...
        if st.button("Next question"):
            if not ok:
                ...
                try:
                    q_text = _generate_question(sel_obj, d_current, model_name=model_name)
        ...
        if st.session_state.interview.get("current_question"):
            ...
            if st.button("Submit answer"):
                if not ok:
                    ...
                    st.error("Please paste a candidate answer.")
                else:
                    ...
                    st.session_state.interview["last_objective_id"] = selected_id
                    st.session_state.interview["current_question"] = None
        ...
        for oid, obj in objs_by_id.items():
            ...
            if not st_obj:
                continue
            active_flag = "ACTIVE" if oid in st.session_state.interview["active_ids"] else "DONE"
            lines.append(
                f"- {obj.get('title','')} | C={float(st_obj.get('C',0.0)):.2f} | "
                f"T={int(st_obj.get('Tspent',0))}/{int(st_obj.get('Tcap',0))}s | {active_flag}"
            )
