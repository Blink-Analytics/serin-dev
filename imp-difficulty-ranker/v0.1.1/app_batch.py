        ...
        if not api_key:
            ...
            if st.button("🚀 Start Batch Scoring", type="primary", use_container_width=True):
                ...
                for idx, row in df.iterrows():
                    ...
                    objectives_raw = str(row[obj_col])
                    
                    try:
                        # Try to parse as JSON
                        # Format: [{"text1"}, {"text2"}] or ["text1", "text2"]
                        # Need to extract the actual text values
                        
                        # Clean up the string
                        objectives_raw = objectives_raw.strip()
                        ...
                        prompt = generate_interview_scoring_prompt_v4_production(
                            job_desc,
                            objectives_list
                        )
                        ...
                        while attempt < max_retries and not success:
                            try:
                                ...
                                try:
                                    ...
                                    st.write(f"DEBUG: LLM returned type: {type(response_parsed)}")
                                    if isinstance(response_parsed, dict):
                                        st.write(f"DEBUG: Dict keys: {list(response_parsed.keys())}")
                                    st.write(f"DEBUG: Raw response (first 500 chars): {response_text[:500]}...")
                                    ...
                                    if isinstance(response_parsed, list):
                                        ...
                                        if "scores" in response_parsed:
                                            ...
                                            if all(k.isdigit() for k in response_parsed.keys()):
                            ...
                            if not success and attempt < max_retries:
                                # Handle rate limits
                                if error_msg and ("rate_limit" in error_msg.lower() or "429" in error_msg):
                        ...
                        # Store results
                        if success and scores_list:
                            importance_scores = [score.get('importance', -1) for score in scores_list]
                            difficulty_scores = [score.get('difficulty', -1) for score in scores_list]
                            ...
                            results.append({
                                'row_index': idx,
                                'LLM_Importance': json.dumps([-1] * len(objectives_list)),
                                'LLM_Difficulty': json.dumps([-1] * len(objectives_list)),
                ...
                if errors > 0:
                    with st.expander(f"⚠️ Error Log ({errors} errors)"):
                        ...
                        st.dataframe(
                            error_rows[['ID', obj_col, "LLM_Error"]],
                            use_container_width=True
                        )
